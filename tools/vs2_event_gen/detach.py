...
Identical in shape and intent to ``tools/vs2_scene_gen/detach.py`` (see that
module's docstring for the full rationale): strips both the trailing
``# blocks: ...`` blob comment and the banner's "generated, do not edit"
language, leaving exactly the body -- ordinary Python indistinguishable from
a file that was always hand-written. One-way: this module's ``checksum``
uses its own ``# blocks:``-shaped :data:`checksum.BLOB_RE`, so
...
record, and once the shape is gone, it is gone.
"""
